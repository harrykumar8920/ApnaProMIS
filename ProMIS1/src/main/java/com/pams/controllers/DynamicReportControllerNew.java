...
@Controller
public class DynamicReportControllerNew {
	...
	public String showReportForm(Model model) {
		...
		return "report/dynamic-reportnew1";
	}
	
	@GetMapping("/getCaseStatus")
	@ResponseBody
	public List<Status> getCaseStatus(@RequestParam Integer caseType) {

	    if (caseType == 1) {
	        // NCLT
	    	List<Status> StatusList1 = addStatusRepo.findAllByTypeAndIsActiveOrderByStatusNameAsc("nclt",true);
	        return StatusList1;
	    } else if (caseType == 2) {
	    	List<Status> StatusList1 = addStatusRepo.findAllByTypeAndIsActiveOrderByStatusNameAsc("C",true);
	        // Criminal
	...
	@PostMapping("/dynamic-reportnew")
	public String generateDynamicReportnew(@ModelAttribute DynamicReportRequestDTONew dto1, Model model) throws Exception {
		
		UserDetails userdet = useDetailRepo.findAllByEmail(userDetailsService.getUserDetails().getUserName());
		
		if (userdet.getId() != 11L && userdet.getId() != 19L)
		dto1.setCreatedBy(userdet);

		List<Object[]> reportList = dynamicReportService.getDynamicReport1(dto1);
		model.addAttribute("statel", stateRepo.findAllByOrderByStateAsc());
		List<Status> StatusList1 = addStatusRepo.findAllByTypeAndIsActiveOrderByStatusNameAsc("C",true);
		List<AddAct> allact = act.findAllByOrderByIdAsc();
	...
	@PostMapping("/dynamic-reportanew")
	public String generateDynamicReport(@ModelAttribute DynamicReportRequestDTONew dto1, Model model) {
		...
		List<Map<String, Object>> reportData = dynamicReportService.generateDynamicReport1(dto);
	...
	@PostMapping("/dynamic-report1new")
	public void generateDynamicReport(
		...
		if ("PDF".equalsIgnoreCase(exportType)) {
			...
			exportPDFnew(columns, tableData, response);
			...
			exportExcelnew(columns, tableData, response);
		} else {
			// Default HTML view
			model.addAttribute("columns", columns);
			model.addAttribute("data", tableData);
	...
	private void exportPDFnew(List<String> columns, List<List<Object>> tableData, HttpServletResponse response)
			throws Exception {
	...
	private void exportExcelnew(List<String> columns, List<List<Object>> tableData, HttpServletResponse response)
			throws Exception {
